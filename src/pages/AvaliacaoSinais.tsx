    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-8 py-12 md:py-20">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          ...
          <motion.div variants={staggerChild} className="max-w-3xl space-y-4 text-muted-foreground mb-8">
            <p className="text-sm">É o equilíbrio entre <span className="text-foreground/80">produção metabólica de calor</span> e <span className="text-foreground/80">perda térmica para o ambiente.</span></p>
            <p className="text-sm">O centro regulador está no <span className="text-foreground/80 font-medium">hipotálamo.</span> Quando há infecção, o corpo aumenta o "set point" térmico.</p>
            <p className="text-sm">Isso causa: <span className="text-foreground/80">tremores, vasoconstrição, sensação de frio.</span> Depois que atinge a nova meta, surge calor e sudorese.</p>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-red-950/30 to-background border border-red-800/20 p-6 md:p-8 rounded-sm mb-4">
            <h3 className="text-lg font-bold text-foreground mb-4">Faixas Numéricas Completas</h3>
            <div className="space-y-2">
              {[
                ...
                <div key={row.situacao} className={`flex items-center justify-between ${row.bg} border border-white/5 px-4 py-2.5 rounded-sm`}>
                  <span className={`text-sm font-medium ${row.color}`}>{row.situacao}</span>
                  <span className="text-sm text-foreground/80 font-mono">{row.valor}</span>
                </div>
              ))}
            </div>
          </motion.div>

          {/* O que a febre indica */}
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-amber-950/20 to-background border border-amber-800/15 p-6 md:p-8 rounded-sm mb-4">
            <h3 className="text-lg font-bold text-foreground mb-4">O que a febre pode indicar?</h3>
          ...
          {/* Sinais de alerta */}
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-red-950/30 to-background border border-red-600/30 p-6 md:p-8 rounded-sm mb-4">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {['Confusão mental', 'Rigidez de nuca', 'Manchas na pele', 'Convulsão', 'Prostração extrema', 'Febre em bebê < 3 meses'].map((s) => (
                <div key={s} className="flex items-center gap-2 text-sm bg-red-500/10 border border-red-500/15 p-3 rounded-sm">
          ...
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-green-950/20 to-background border border-green-800/15 p-6 md:p-8 rounded-sm">
            <h3 className="text-lg font-bold text-foreground mb-4">Como Medir Corretamente</h3>
            <div className="grid sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={m.local} className="bg-white/5 border border-white/10 p-4 rounded-sm">
                  <h4 className="font-bold text-foreground text-sm mb-2">{m.local}</h4>
                  {m.passos.map((p, i) => (
                    <div key={i} className="flex items-start gap-2 text-xs text-foreground/70 mb-1">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          <motion.div variants={staggerChild} className="mb-6">
            ...
            <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Frequência Cardíaca</h2>
            <p className="text-muted-foreground text-sm">Perfusão e Compensação</p>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="max-w-3xl space-y-3 text-muted-foreground mb-8">
          ...
          {/* Faixas por idade */}
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-rose-950/30 to-background border border-rose-800/20 p-6 md:p-8 rounded-sm mb-4">
            <h3 className="text-lg font-bold text-foreground mb-4">Faixas por Idade</h3>
            <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <div key={f.grupo} className="bg-white/5 border border-white/10 p-4 rounded-sm text-center">
                  <span className="text-xs font-mono uppercase tracking-wider text-muted-foreground">{f.grupo}</span>
          ...
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-red-950/30 to-background border border-red-600/30 p-6 md:p-8 rounded-sm mb-4">
            ...
            <p className="text-xs text-muted-foreground font-mono mb-2">Pode indicar:</p>
            <div className="grid sm:grid-cols-2 gap-2">
          ...
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-purple-950/20 to-background border border-purple-800/15 p-6 md:p-8 rounded-sm">
            <h3 className="text-lg font-bold text-foreground mb-4">Como Avaliar Perfusão</h3>
            <div className="space-y-2 mb-4">
              {[
                'Pressione a unha por 2 segundos.',
                'Solte.',
                'A cor deve retornar em até 2 segundos.',
              ].map((step, i) => (
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-sm">
              <p className="text-xs text-foreground/80">Se demorar mais → <span className="text-amber-400 font-semibold">possível baixa perfusão.</span></p>
            </div>
          </motion.div>
        </motion.section>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          <motion.div variants={staggerChild} className="mb-6">
            ...
            <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Frequência Respiratória</h2>
            <p className="text-muted-foreground text-sm">O sinal mais subestimado</p>
          </motion.div>

          <motion.div variants={staggerChild} className="max-w-3xl mb-8">
            <div className="bg-sky-500/10 border border-sky-500/20 p-4 rounded-sm">
              <p className="text-sm text-foreground/80 font-medium">Respiração altera <span className="text-sky-400 font-bold">antes do pulso.</span></p>
            </div>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-sky-950/30 to-background border border-sky-800/20 p-6 md:p-8 rounded-sm mb-4">
            <h3 className="text-lg font-bold text-foreground mb-4">Valores Normais</h3>
          ...
          {/* Taquipneia */}
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-amber-950/20 to-background border border-amber-800/15 p-6 md:p-8 rounded-sm mb-4">
            <h3 className="text-lg font-bold text-foreground mb-2">Taquipneia (respiração acelerada)</h3>
            <p className="text-sm text-foreground/80 mb-4">Adulto <span className="text-amber-400 font-bold">&gt; 24 rpm</span> em repouso</p>
            <p className="text-xs text-muted-foreground font-mono mb-2">Pode indicar:</p>
            <div className="grid sm:grid-cols-2 gap-2">
          ...
          {/* Sinais de alerta */}
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-red-950/30 to-background border border-red-600/30 p-6 md:p-8 rounded-sm">
            <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
              <AlertTriangle size={18} className="text-red-400" />
            ...
            <div className="space-y-2">
              {[
                'Uso de musculatura do pescoço',
                'Retração intercostal',
                'Lábios arroxeados',
                'Pausas respiratórias',
                'Gemido respiratório em criança',
              ].map((s) => (
                <div key={s} className="flex items-center gap-2 text-sm bg-red-500/10 border border-red-500/15 p-3 rounded-sm">
                  <AlertTriangle size={12} className="text-red-400 shrink-0" />
                  <span className="text-foreground/80">{s}</span>
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          <motion.div variants={staggerChild} className="mb-6">
            ...
            <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Desidratação</h2>
            <p className="text-muted-foreground text-sm">Volume Circulante</p>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="max-w-3xl mb-8">
            <p className="text-sm text-muted-foreground mb-3">Água mantém:</p>
            <div className="grid grid-cols-2 gap-2">
          ...
          <motion.div variants={staggerChild} className="grid sm:grid-cols-3 gap-4 mb-4">
            {[
              {
                grau: 'Grau Leve (3–5%)',
                sintomas: ['Sede', 'Boca seca', 'Urina amarela'],
                color: 'from-yellow-500/20 to-yellow-600/10',
                border: 'border-yellow-500/30',
                accent: 'text-yellow-400',
              },
              {
                grau: 'Grau Moderado (6–8%)',
                sintomas: ['Tontura', 'Taquicardia', 'Pele menos elástica', 'Redução de urina'],
              ...
              {
                grau: 'Grau Grave (>10%)',
                sintomas: ['Confusão', 'Extremidades frias', 'Hipotensão', 'Letargia'],
                color: 'from-red-500/20 to-red-600/10',
                border: 'border-red-500/30',
                accent: 'text-red-400',
              },
            ].map((g) => (
              <div key={g.grau} className={`bg-gradient-to-br ${g.color} border ${g.border} p-5 rounded-sm`}>
                <h4 className={`font-bold mb-3 ${g.accent}`}>{g.grau}</h4>
                {g.sintomas.map((s) => (
                  <div key={s} className="flex items-center gap-2 text-sm text-foreground/70 mb-1">
          ...
          {/* Testes funcionais */}
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-blue-950/20 to-background border border-blue-800/15 p-6 md:p-8 rounded-sm">
            <h3 className="text-lg font-bold text-foreground mb-4">Testes Funcionais</h3>
            <div className="grid sm:grid-cols-2 gap-2">
              {['Elasticidade da pele', 'Frequência urinária', 'Cor da urina', 'Perfusão capilar'].map((t) => (
                <div key={t} className="flex items-center gap-2 text-sm bg-blue-500/10 border border-blue-500/15 p-3 rounded-sm">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          <motion.div variants={staggerChild} className="mb-6">
            ...
            <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Nível de Consciência</h2>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="max-w-3xl mb-8">
            <p className="text-sm text-muted-foreground mb-4">Avaliar: A pessoa está alerta? Confusa? Sonolenta? Não responde?</p>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="grid sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {[
              ...
              <div key={level.letra} className={`bg-gradient-to-br ${level.color} border ${level.border} p-5 rounded-sm text-center`}>
                <span className={`text-3xl font-black ${level.accent}`}>{level.letra}</span>
                <p className="text-sm font-bold text-foreground mt-2">{level.nome}</p>
                <p className="text-xs text-muted-foreground mt-1">{level.desc}</p>
              </div>
            ))}
          </motion.div>

          <motion.div variants={staggerChild} className="bg-red-500/10 border border-red-500/20 p-4 rounded-sm">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          <motion.div variants={staggerChild} className="mb-6">
            <span className="pre-title">Pilar 06</span>
            <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Critérios Objetivos para Decisão</h2>
          </motion.div>

          <motion.div variants={staggerChild} className="bg-gradient-to-br from-red-950/40 to-background border border-red-600/30 p-6 md:p-8 rounded-sm mb-4">
            <h3 className="text-lg font-bold text-foreground mb-4 flex items-center gap-2">
              <AlertTriangle size={18} className="text-red-400" />
            ...
            <div className="space-y-2">
              {[
                'Temperatura ≥ 40°C',
                'Pulso ≥ 130 bpm em repouso',
                'Respiração ≥ 30 rpm em adulto',
                'Perfusão capilar > 3 segundos',
                'Confusão mental',
                'Desidratação grave',
                'Convulsão',
              ].map((c) => (
                <div key={c} className="flex items-center gap-2 text-sm bg-red-500/10 border border-red-500/15 p-3 rounded-sm">
        ...
        <motion.section initial="hidden" whileInView="visible" viewport={viewportOnce} variants={stagger(0.08)} className="mb-24">
          <motion.div variants={staggerChild} className="mb-6">
            <span className="pre-title">Registro</span>
            <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-2">Registro e Evolução</h2>
          </motion.div>
          ...
          <motion.div variants={staggerChild} className="bg-gradient-to-br from-green-950/20 to-background border border-green-800/15 p-6 md:p-8 rounded-sm mb-4">
            <p className="text-sm text-muted-foreground mb-4">Monitorar evolução permite identificar tendência.</p>
            <h3 className="text-lg font-bold text-foreground mb-4">Registrar:</h3>
            <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3">
              {['Data', 'Hora', 'Temperatura', 'Pulso', 'Respiração', 'Hidratação', 'Consciência'].map((field) => (
                <div key={field} className="flex items-center gap-2 text-sm bg-green-500/10 border border-green-500/15 p-3 rounded-sm">
                  <ClipboardList size={14} className="text-green-400 shrink-0" />
                  <span className="text-foreground/80">{field}</span>
                </div>
              ))}
            </div>
            <p className="text-xs text-muted-foreground mt-4 border-t border-white/5 pt-3 flex items-center gap-2">
        ...
        {/* ─── CONCLUSÃO ─── */}
        <motion.div initial="hidden" whileInView="visible" viewport={viewportOnce} variants={fadeUp} className="bg-sky-500/10 border border-sky-500/20 p-6 rounded-sm mb-6">
          ...
          <p className="text-sm text-foreground/80 mt-2">Você passa a enxergar padrões. E <span className="text-green-400 font-bold">padrões salvam tempo.</span></p>
        </motion.div>
        ...
        <div className="flex items-center justify-between pt-8 border-t border-border">
          ...
          <Link to="/projeto-autonomo/primeiros-socorros" className="text-muted-foreground hover:text-foreground transition-colors text-sm font-mono flex items-center gap-2">
            Primeiros Socorros
